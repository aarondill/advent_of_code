...
public class Part1 {
  ...
  public static boolean isAntinode(int x, int y, Map<Character, List<Point>> antennae) {
    ...
    for (List<Point> points : antennae.values()) {
      // the angle to two points is the same and the distance of one is 2x the other, we are in the antennae
      for (int i = 0; i < points.size(); i++) {
        for (int j = i + 1; j < points.size(); j++) {
          // System.out.printf("Testing(%d,%d): %s, %s\n", i, j, points.get(i), points.get(j));
          Point p1 = points.get(i), p2 = points.get(j);
          // Check distance (between = closer)
          double distBetweenPoints = Math.hypot(p2.x() - p1.x(), p2.y() - p1.y());
          double distToClosestPoint = Math.min(Math.hypot(p1.x() - x, p1.y() - y), Math.hypot(p2.x() - x, p2.y() - y));
          // System.out.printf("Dist: %f, %f\n", distBetweenPoints, distToClosestPoint);
          if (distBetweenPoints != distToClosestPoint) continue;
